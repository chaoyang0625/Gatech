...
class PageRank(object):     
    ...
    def initAllStructs(self):
        ...
        self.inList = [[] for _ in range(self.N)]
        if self.selfLoops:
            i = 0
            self.outDegree = [1 for _ in range(self.N)]
            for node, out_links in self.adjList.items():
                self.outDegree[node] = len(out_links)+1
                self.inList
                for link in out_links:
                    self.inList[link].append(node)
            while(i<len(self.inList)):
                self.inList[i].append(i)
                i += 1
            self.rankVec = [float(1/self.N) for _ in range(self.N)]
        else:
            self.outDegree = [0 for _ in range(self.N)]
            for node, out_links in self.adjList.items():
                self.outDegree[node] = len(out_links)
                for link in out_links:
                    self.inList[link].append(node)
    ...
    def solveRankIter(self, oldRankVec):
        ...
        if self.selfLoops==False:
            ...
            while (z<len(self.sinkNodes)):
                danglesum += self.alpha* oldRankVec[self.sinkNodes[z]]
                z += 1
            danglesumNor = danglesum/self.N + self.damping_value
            while(i<self.N):
                rank = danglesumNor
                for j in self.inList[i]:
                    rank += self.alpha * oldRankVec[j] / self.outDegree[j]
                newRankVec[i] = rank
                if(abs(newRankVec[i]-oldRankVec[i])>self.flag):
                    self.flag = abs(newRankVec[i]-oldRankVec[i])
                i += 1
        #type 1
        else:
            i = 0
