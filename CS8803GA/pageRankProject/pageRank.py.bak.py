...
class PageRank(object):     
    ...
    def __init__(self, inFileName, alpha=.85, selfLoops=False):
        ...
        self.inList = defaultdict(list)
        self.outDegree = {}
        self.sinkNodes = []
        #Task 1 : initialize data structures you will use
        self.flag = 0
        self.damping_value = (1.0 - self.alpha) / self.N
        self.initAllStructs()   
    ...
    def initAllStructs(self):
        ...
        print (self.selfLoops)
        # Type 3
        if self.selfLoops==False:
            print (len(self.nodeIDs))
            #initialize self.rankVec
            self.rankVec = [1.0/self.N]*(len(self.nodeIDs))
            #conditionally build list of all sink nodes
            self.sinkNodes = list(set(self.nodeIDs).difference(set(self.adjList.keys())))
            print ("step 1")
            # build in-list and out-dergee
            
            for x in self.adjList:
                for outNode in self.adjList[x]:
                    if outNode in self.inList.keys():
                        self.inList[outNode].append(x)
                    else:
                        self.inList.update({outNode:[x]})
                self.outDegree.update({x:self.alpha * 1.0/len(self.adjList[x])})
        
        #Type 1
        else:
            print (len(self.nodeIDs))
            self.rankVec = [1.0/self.N]*(len(self.nodeIDs))
            #[self.adjList[i].append(i) for i in self.nodeIDs]
            i=0
            while (i<len(self.nodeIDs)):
                
                if self.nodeIDs[i] not in self.adjList.keys():
                    self.adjList.update({self.nodeIDs[i]:[self.nodeIDs[i]]})
                    #self.sinkNodes.append(i)
                else:
                    self.adjList[self.nodeIDs[i]].append(self.nodeIDs[i])
                i += 1
            #print (len(self.nodeIDs))
            for x, n in self.adjList.items():
                ...
                for outNode in n:
                    if outNode in self.inList.keys():
                        ...
                        self.inList.update({outNode:[x]})
                self.outDegree.update({x:sself.alpha * 1.0/len(n)})
    ...
    def solveRankIter(self, oldRankVec):
        ...
        self.flag = 0
        if self.selfLoops == False:
            #danglesum = self.alpha * sum(oldRankVec[self.nodeIDs.index(nodes)] for nodes in self.sinkNodes)
            i=0
            z=0
            danglesum = 0
            while (z<len(self.sinkNodes)):
                danglesum += self.alpha* oldRankVec[self.nodeIDs.index(self.sinkNodes[z])]
                z += 1
            danglesumNor = danglesum/self.N + self.damping_value
            while(i< self.N):
                rank = danglesumNor
                j=0
                while(j<len(self.inList[self.nodeIDs[i]])):
                    rank += (newRankVec[self.nodeIDs.index(self.inList[i][j])]*self.outDegree[self.inList[i][j]])
                    j += 1
                newRankVec[i] = rank
                i += 1
            self.flag = np.max(list(map(lambda i: abs(i[0]-i[1]), zip(newRankVec, oldRankVec) )))
            ...
            while(i< len(self.nodeIDs)):
                ...
                j=0
                while(j<len(self.inList[self.nodeIDs[i]])):
                    rank += self.alpha*(newRankVec[self.nodeIDs.index(self.inList[i][j])]*self.outDegree[self.inList[i][j]])
                    j += 1
                rank += self.damping_value
                newRankVec[i] = rank
                i += 1
            self.flag = np.max(list(map(lambda i: abs(i[0]-i[1]), zip(newRankVec, oldRankVec) )))
        return newRankVec
